...
@st.cache_resource
def load_models():
    # Updated paths to match the actual location of model files
    flag_model = joblib.load("invoice_flagging/models/random_forest.pkl")
    scaler = joblib.load("invoice_flagging/models/scaler.pkl")
    freight_model = joblib.load("freight_cost_prediction/models/predict_freight_model.pkl")
...
with tab2:
    ...
    if st.button("Predict Freight Cost", type="primary", use_container_width=True):
        ...
        if dollars > 0:
            ...
            st.caption("Invoice amount must be greater than $0 for percentage calculation.")
